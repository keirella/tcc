...
const dashStyles = `
  ...
  .dash-main { flex: 1; min-width: 0; display: flex; flex-direction: column; min-height: 100vh; box-sizing: border-box; }
  
  .dash-topbar { background: white; padding: 0 clamp(16px, 3vw, 40px); height: 64px; display: flex; align-items: center; justify-content: space-between; border-bottom: 1px solid rgba(131,153,88,0.12); position: sticky; top: 0; z-index: 50; box-shadow: 0 1px 4px rgba(10,51,35,0.06); width: 100%; box-sizing: border-box; }
  .dash-topbar-title { font-size: 17px; font-weight: 700; color: #0A3323; display: flex; align-items: center; gap: 8px; }
  .dash-topbar-right { display: flex; align-items: center; gap: 12px; }
  
  .dash-container { flex: 1; padding: 24px clamp(16px, 3vw, 40px) clamp(24px, 3vw, 40px); box-sizing: border-box; overflow-x: hidden; }
  
  .dash-welcome-section { display: flex; align-items: center; justify-content: space-between; margin-bottom: 28px; flex-wrap: wrap; gap: 14px; }
  .dash-welcome-text h1 { font-size: clamp(20px, 2.5vw, 26px); font-weight: 800; color: ${COLORS.darkGreen}; margin: 0; }
  .dash-welcome-text p { color: ${COLORS.mossGreen}; font-size: 13px; margin: 4px 0 0; font-weight: 500; }
  ...
  .stat-card { background: white; border-radius: 16px; padding: 22px 20px; position: relative; overflow: hidden; transition: transform 0.2s, box-shadow 0.2s; box-shadow: 0 2px 12px rgba(10,51,35,0.06); }
  .stat-card:hover { transform: translateY(-3px); box-shadow: 0 6px 20px rgba(10,51,35,0.1); }
  ...
  .stat-value  { font-size: 26px; font-weight: 800; color: ${COLORS.darkGreen}; line-height: 1; margin-bottom: 6px; }
  ...
  .seller-notif-panel { position: absolute; top: 54px; right: 0; width: 320px; background: white; border-radius: 18px; box-shadow: 0 8px 32px rgba(10,51,35,0.15); z-index: 200; overflow: hidden; animation: dashPopIn 0.2s ease; }
  @keyframes dashPopIn { from { transform: scale(0.95) translateY(-8px); opacity: 0; } to { transform: scale(1) translateY(0); opacity: 1; } }
  ...
  .seller-notif-toast { position: fixed; top: 80px; right: 24px; z-index: 999; max-width: 340px; background: white; border-radius: 16px; box-shadow: 0 8px 32px rgba(10,51,35,0.18); padding: 14px 18px; display: flex; align-items: flex-start; gap: 12px; border-left: 4px solid #839958; animation: sellerSlideIn 0.3s ease; }
  ...
  .dash-loading { display: flex; align-items: center; justify-content: center; height: 100vh; font-size: 15px; color: ${COLORS.mossGreen}; font-family: 'Poppins', sans-serif; background-color: ${COLORS.beige}; }

  /* ── Responsive breakpoints ── */
  @media (max-width: 1100px) {
    .seller-sidebar { width: 200px; }
    .dash-stats-grid { grid-template-columns: repeat(auto-fit, minmax(160px, 1fr)); }
  }
  ...
  @media (max-width: 600px) {
    ...
    .dash-welcome-section { flex-direction: column; align-items: flex-start; gap: 10px; }
                    ...
                    <div className="menu-details" style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 4 }}>
                        <div style={{ flex: 1, height: 4, background: "#f0ede0", borderRadius: 99, overflow: "hidden" }}>
                          <div style={{ height: "100%", width: `${(m.sold / maxSold) * 100}%`, background: idx === 0 ? COLORS.darkGreen : COLORS.mossGreen, borderRadius: 99 }} />
                        </div>
