...
import { login } from "../../services/api"; // aktifkan setelah API jalan
...
const css = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .login-left::before {
    ...
    background: rgba(247,244,213,0.12); border-radius: 50%;
  }
  .login-left::after {
    content: ''; position: absolute;
    bottom: -60px; left: -60px;
    width: 240px; height: 240px;
    background: rgba(16,86,102,0.12); border-radius: 50%;
  }
  ...
  /* ROLE TOGGLE */
  .role-toggle {
    display: flex; background: ${COLORS.bg_light};
  ...
  .role-btn {
    ...
    color: rgba(16,86,102,0.55); background: transparent;
  }
  ...
  .form-input {
    ...
    font-family: 'Poppins', sans-serif; color: ${COLORS.text_dark};
    background: ${COLORS.bg_light}; outline: none;
...
export default function Login({ onLoginSuccess, onGoToRegister }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // ── DUMMY LOGIN (hapus setelah API jalan) ──────────────────
      await new Promise(r => setTimeout(r, 800)); // simulasi loading

      if (role === "buyer") {
        if (email === "budi@gmail.com" && password === "12345678") {
          onLoginSuccess({ id: 7, name: "Budi Santoso", email, role: "buyer" });
        } else {
          throw new Error("Email atau password salah");
        }
      } else {
        if (kodeUnik === "STAN001" && password === "password123") {
          onLoginSuccess({ id: 1, name: "Seller Padang", kode_unik: "STAN001", role: "seller", stall_id: 1 });
        } else if (kodeUnik === "STAN002" && password === "password123") {
          onLoginSuccess({ id: 2, name: "Seller Ayam Geprek", kode_unik: "STAN002", role: "seller", stall_id: 2 });
        } else {
          throw new Error("Kode unik atau password salah");
        }
      }
      // ── AKTIFKAN INI SETELAH API JALAN ────────────────────────
        ...
        <div className="login-left">
          <div className="login-left-content">
            ...
            <div className="login-left-sub">
              Pesan makanan dari berbagai stan kantin, bayar sekali, tanpa antre panjang.
            </div>
          ...
          <div className="login-right-inner">
            ...
            {role === "buyer" && (
              <div className="login-footer">
                Belum punya akun?{" "}
                <a onClick={onGoToRegister}>Daftar sekarang</a>
              </div>
            )}
            {role === "seller" && (
              <div className="login-footer">
                Belum punya akun penjual?{" "}
                <a>Hubungi pengelola kantin</a>
              </div>
            )}
          </div>
        </div>
