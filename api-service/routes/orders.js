...
const STATUS_NOTIF = {
    pending:   { title: "⏳ Menunggu Konfirmasi",  body: "Pesananmu masuk, menunggu konfirmasi pembayaran." },
    paid:      { title: "✅ Pembayaran Diterima",   body: "Pembayaran dikonfirmasi! Pesananmu segera dimasak." },
    cooking:   { title: "🍳 Sedang Dimasak",        body: "Pesananmu sedang dimasak. Tunggu sebentar ya!" },
    ready:     { title: "🎉 Siap Diambil!",         body: "Pesananmu sudah siap. Ambil di stan sekarang!" },
...
async function sendOrderNotif(orderId, status) {
    ...
    try {
        ...
        console.log(`[NOTIF] Order #${orderId} → buyer:`, rows[0]?.buyer_id, rows[0]?.name, '| has token:', !!rows[0]?.fcm_token);
